...
public interface UserAccountRepository extends JpaRepository<UserAccountEntity, Integer> {
    ...
    @Query("SELECT new com.example.backend4rate.models.dto.User(" +
            "su.id, ua.id as userAccountId, ua.username, ua.role, ua.status, ua.confirmed as confirmed, ua.email, " +
    ...
    @Query("SELECT new com.example.backend4rate.models.dto.User(" +
            "a.id, ua.id as userAccountId, ua.username, ua.role, ua.status, ua.confirmed as confirmed, ua.email, " +
    ...
    User getAdminUserByUserAccountId(@Param("userAccountId") Integer userAccountId);
    ...
    @Query("SELECT a.userAccount FROM AdministratorEntity a WHERE a.id = :administratorId")
    UserAccountEntity getUserAccountByAdministratorId(@Param("administratorId") Integer administratorId);
}
