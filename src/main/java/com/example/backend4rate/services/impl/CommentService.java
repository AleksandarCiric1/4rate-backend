...
public class CommentService implements CommentServiceInterface {
    ...
    public CommentService(CommentRepository commentRepository,RestaurantRepository restaurantRepository, GuestRepository guestRepository, StandardUserRepository standardUserRepository,UserAccountRepository userAccountRepository, ModelMapper modelMapper){
    ...
    public Comment getCommentById(Integer commentId) throws NotFoundException{
        return modelMapper.map(commentRepository.findById(commentId).orElseThrow(NotFoundException::new), Comment.class);
    }

    @Override
    public List<Comment> getAllComments(Integer restaurantId) throws NotFoundException {
        RestaurantEntity restaurantEntity= restaurantRepository.findById(restaurantId).orElseThrow(NotFoundException::new);
        return commentRepository.findByRestaurant(restaurantEntity).stream().map(l -> modelMapper.map(l, Comment.class)).collect(Collectors.toList());
    }
