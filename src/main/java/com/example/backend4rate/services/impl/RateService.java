...
public class RateService implements RateServiceInterface{
    ...
    public RateService(GradeRepository gradeRepository, RestaurantRepository restaurantRepository, UserAccountRepository userAccountRepository, StandardUserRepository standardUserRepository, GuestRepository guestRepository, GuestRepository guestRepository2, GradeRepository gradeRepository2, ModelMapper modelMapper,EntityManager entityManager)
    {
    ...
    public Grade addRating(Grade grade) throws InvalidGradeException, NotFoundException {
        if(grade.getValue()>5 || grade.getValue()<1) throw new InvalidGradeException("Grade " +  grade.getValue() + " is not in allowed range!");
        if(!guestRepository.findById(grade.getGuestId()).isPresent()) throw new NotFoundException();
        if(!restaurantRepository.findById(grade.getRestaurantId()).isPresent()) throw new NotFoundException();
        Integer vGrade=null;
