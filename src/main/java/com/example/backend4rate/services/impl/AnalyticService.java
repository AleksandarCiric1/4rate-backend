...
public class AnalyticService implements AnalyticServiceInterface {
    ...
    public List<UsersPerMonth> getUserCreationStatsForLastYear() {
        ...
        List<UserAccountEntity> filteredUsers = allUsers.stream()
                .filter(user -> {
                    LocalDateTime userCreatedAt = user.getCreatedAt().toInstant().atZone(ZoneId.systemDefault())
                            .toLocalDateTime();
        ...
        Map<YearMonth, Long> aggregatedData = filteredUsers.stream()
                .collect(Collectors.groupingBy(
                        user -> {
                            LocalDateTime dateTime = user.getCreatedAt().toInstant().atZone(ZoneId.systemDefault())
                                    .toLocalDateTime();
        ...
        return aggregatedData.entrySet().stream()
                ...
                .sorted((a, b) -> {
                    ...
                    return yearComparison != 0 ? yearComparison : Integer.compare(a.getMonth(), b.getMonth());
                })
                .collect(Collectors.toList());
    }
}
